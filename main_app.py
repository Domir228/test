...
class InstrScr (Screen):
    def __init__(self, **kwargs):
        ...
        lb11 = Label(text='Bвeдитe имя:', halign='right') 
        self.in_name = TextInput (multiline=False)
        lb12 = Label(text='Введите возраст:', halign='right') 
        self.in_age = TextInput (text='7', multiline=False)
        self.btn = Button(text='Haчaть', size_hint=(0.3, 0.2), pos_hint={'center_x':5})
        self.btn.on_press = self.next                                    
        line1 = BoxLayout(size_hint=(0.8, None), height='30sp') 
        line2 = BoxLayout(size_hint=(0.8, None), height='30sp')
        line1.add_widget(lb11)
        line1.add_widget(self.in_name)
        line2.add_widget(lb12)
        line2.add_widget(self.in_age)
        outer = BoxLayout (orientation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
        outer.add_widget(line1)
        outer.add_widget(line2)
        self.btn.background_color = (0.9, 0, 0, 0)
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        self.run = Runner (total=38, steptime=1.5, size_hint=(0.4, 1)) 
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        #Lbl1 = Label(text='Cчumaŭme пульс')
        self.lbl_sec = Seconds(15)
        self.lbl_sec.bind(done=self.sec_finished)
        line = BoxLayout(size_hint=(0.8, None), height='30sp')
        lbl_result = Label (text='Bвeдитe результат:', halign='right')
        self.in_result = TextInput(text='0', multiline=False)
        self.in_result.set_disabled(True)
        line.add_widget(lbl_result)
        line.add_widget(self.in_result)
        self.btn = Button(text='Haчaть', size_hint=(0.3, 0.4), pos_hint={'center_x': 0.5})
        self.btn.on_press = self.next
        outer = BoxLayout (orientation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
...
class CheckSits (Screen):
    def __init__(self, **kwargs):
        ...
        self.btn = Button(text='Пpoдoлжить', size_hint=(0.3, 0.2), pos_hint={'center_x': 0})
        self.btn.on_press = self.next
        outer = BoxLayout (orientation='vertical', padding=8, spacing=8) 
        outer.add_widget(instr)
        outer.add_widget(self.btn) 
...
class PulseScr2(Screen):
    def __init__(self, **kwargs): 
        ...
        line2 = BoxLayout(size_hint=(0.8, None), height='30sp')
        lbl_result2 = Label (text='Результат после отдыжа:', halign='right') 
        self.in_result2 = TextInput(text='0', multiline=False)
        self.in_result1.set_disabled(True)
        self.in_result2.set_disabled (True)
        line2.add_widget(lbl_result2)
        line2.add_widget(self.in_result2)
        self.btn = Button(text='Haчaть', size_hint=(0.3, 0.5), pos_hint={'center_x': 0.5})
        self.btn.on_press = self.next
        outer = BoxLayout (orientation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
    ...
    def sec_finished (self, *args): 
        if self.lbl_sec.done:
            if self.stage == 0:
            # закончили первый подсчёт, отдыхаем self.stage = 1
